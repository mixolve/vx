...
void VxAudioProcessorEditor::syncGlobalInGainLrControlFromParameters()
{
    if (syncingGlobalInGainLrControl || globalInGainLrControl == nullptr)
        return;

    auto* leftParameter = valueTreeState.getRawParameterValue(VxAudioProcessor::paramGlobalGainLId);
    auto* rightParameter = valueTreeState.getRawParameterValue(VxAudioProcessor::paramGlobalGainRId);

    if (leftParameter == nullptr || rightParameter == nullptr)
        return;

    const auto leftValue = leftParameter->load(std::memory_order_relaxed);
    const auto rightValue = rightParameter->load(std::memory_order_relaxed);
    const auto linkedValue = (leftValue + rightValue) * 0.5f;
    const auto linked = std::abs(leftValue - rightValue) < 0.005f;

    juce::ScopedValueSetter<bool> guard(syncingGlobalInGainLrControl, true);
    globalInGainLrControl->setValue(linked ? leftValue : linkedValue, false);

    if (linked)
        globalInGainLrControl->clearOverrideText();
    else
        globalInGainLrControl->setOverrideText("MIXED");
}
...
void VxAudioProcessorEditor::setupShellControls()
{
    ...
    globalInGainLrControl = std::make_unique<LocalParameterControl>("IN-GAIN-LR",
                                                                    2,
                                                                    -48.0,
                                                                    48.0,
                                                                    0.01,
                                                                    0.0);
    globalInGainLrControl->onValueChanged = [this]
    {
        if (syncingGlobalInGainLrControl || globalInGainLrControl == nullptr)
            return;

        const auto value = globalInGainLrControl->getValue();
        juce::ScopedValueSetter<bool> guard(syncingGlobalInGainLrControl, true);
        globalInGainLrControl->clearOverrideText();

        if (gainLControl != nullptr)
            gainLControl->setValue(value, true);

        if (gainRControl != nullptr)
            gainRControl->setValue(value, true);
    };
    addAndMakeVisible(*globalInGainLrControl);

    gainLControl->onValueChanged = [this]
    {
        syncGlobalInGainLrControlFromParameters();
    };
    gainRControl->onValueChanged = [this]
    {
        syncGlobalInGainLrControlFromParameters();
    };
    syncGlobalInGainLrControlFromParameters();
