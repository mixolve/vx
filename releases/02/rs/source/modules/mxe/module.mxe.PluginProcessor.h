...
{
public:
    struct ExternalBandParameters
    {
        std::atomic<float>* bypass = nullptr;
        std::atomic<float>* bypassWithGain = nullptr;
        std::atomic<float>* inGainLr = nullptr;
        std::atomic<float>* inGainL = nullptr;
        std::atomic<float>* inGainR = nullptr;
        std::atomic<float>* wide = nullptr;
        std::atomic<float>* outGain = nullptr;
    };

    enum class ExternalParameterTarget
    {
        moduleBypass,
        moduleBypassWithGain,
        fullbandInGainLr,
        fullbandInGainL,
        fullbandInGainR,
        fullbandWide,
    ...
    struct ExternalParameterChange
    {
        ExternalParameterTarget target = ExternalParameterTarget::moduleBypass;
        size_t bandIndex = 0;
        float value = 0.0f;
    };
    ...
    {
        std::atomic<float>* moduleBypass = nullptr;
        std::atomic<float>* moduleBypassWithGain = nullptr;
        std::atomic<float>* inGainLr = nullptr;
        std::atomic<float>* inGainL = nullptr;
        std::atomic<float>* inGainR = nullptr;
        std::atomic<float>* wide = nullptr;
        std::atomic<float>* outGain = nullptr;
        std::array<ExternalBandParameters, mxe::dsp::MultibandProcessor::numBands> bandParameters {};
    ...
    void setExternalParameterConnection(ExternalParameterConnection connection);
    void clearExternalParameterConnection() noexcept;
    bool syncExternalParameterValueTreeState();
...
private:
    ...
    bool hasExternalParameterConnection() const noexcept;
    float loadExternalFloat(std::atomic<float>* value, float defaultValue) const noexcept;
    bool loadExternalBool(std::atomic<float>* value, bool defaultValue = false) const noexcept;
    ...
    bool mirroredExternalStateSyncInProgress = false;
    ExternalParameterConnection externalParameterConnection {};
