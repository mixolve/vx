                                   ...
                                   const juce::Colour defaultColour)
{
    ...
    while (startIndex < text.length())
    {
        ...
        juce::Colour tokenColour = defaultColour;

        if (token == "L")
        {
            tokenColour = leftLabelColour;
            appendedAnyColour = true;
        }
        else if (token == "R")
        {
            tokenColour = rightLabelColour;
            appendedAnyColour = true;
        }
        else if (token == "UP")
        {
            tokenColour = upLabelColour;
            appendedAnyColour = true;
        }
        else if (token == "DN" || token == "DOWN")
        {
            tokenColour = downLabelColour;
            appendedAnyColour = true;
        }

        attributed.append(token, font, tokenColour);

        if (dotIndex >= 0)
            attributed.append(".", font, defaultColour);

        startIndex = tokenEnd + 1;
    }

    if (! appendedAnyColour)
        return false;
...
public:
    ValueBoxComponent(juce::Slider& sliderToControl,
                                            ...
                                            juce::String parameterIdToControl,
        ...
        : slider(sliderToControl),
                    state(stateToUse),
                    parameterId(std::move(parameterIdToControl)),
...
void BoxTextButton::setTextColour(const juce::Colour colour)
{
    textColour = colour;
    repaint();
}
...
void BoxTextButton::paintButton(juce::Graphics& graphics, bool shouldDrawButtonAsHighlighted, bool shouldDrawButtonAsDown)
{
    ...
    if (drawDirectionalTokenHighlight(graphics, buttonText, textBounds, font, textJustification, textColour))
        return;

    graphics.setColour(textColour);
    graphics.setFont(font);
    graphics.drawFittedText(buttonText, textBounds, textJustification, 1, 1.0f);
}
