      ...
      callbacks(std::move(callbacksIn))
{
    ...
    inGainLrControl = std::make_unique<ParameterControl>(valueTreeState,
                                                         TseModuleProcessor::paramMiscInGainLrId,
                                                         "IN-GAIN-LR",
                                                         1);
    inGainLrControl->setTitleLongPressAction([this]
    {
        resetControl(inGainLrControl.get(), 0.0, [this] { clearFocus(); });
    });
    content.addAndMakeVisible(*inGainLrControl);

    inGainLControl = std::make_unique<ParameterControl>(valueTreeState,
                                                        TseModuleProcessor::paramMiscInGainLId,
                                                        "IN-GAIN-L",
                                                        1);
    inGainLControl->setTitleLongPressAction([this]
    {
        resetControl(inGainLControl.get(), 0.0, [this] { clearFocus(); });
    });
    content.addAndMakeVisible(*inGainLControl);

    inGainRControl = std::make_unique<ParameterControl>(valueTreeState,
                                                        TseModuleProcessor::paramMiscInGainRId,
                                                        "IN-GAIN-R",
                                                        1);
    inGainRControl->setTitleLongPressAction([this]
    {
        resetControl(inGainRControl.get(), 0.0, [this] { clearFocus(); });
    });
    content.addAndMakeVisible(*inGainRControl);
    ...
    {
        resetControl(wideControl.get(), 100.0, [this] { clearFocus(); });
    });
    content.addAndMakeVisible(*wideControl);
...
void TseModuleComponent::resized()
{
    ...
    if (miscExpanded)
    {
        ...
        placeRow(inGainLrControl.get());
        placeRow(inGainLControl.get());
        placeRow(inGainRControl.get());
        placeRow(wideControl.get());
    ...
    includeComponentBounds(miscFrameBounds, inGainLrControl.get());
    includeComponentBounds(miscFrameBounds, inGainLControl.get());
    includeComponentBounds(miscFrameBounds, inGainRControl.get());
    includeComponentBounds(miscFrameBounds, wideControl.get());
    includeComponentBounds(miscFrameBounds, outGainControl.get());
...
void TseModuleComponent::updateSectionVisibility()
{
    ...
    if (inGainLrControl != nullptr) inGainLrControl->setVisible(miscExpanded);
    if (inGainLControl != nullptr) inGainLControl->setVisible(miscExpanded);
    if (inGainRControl != nullptr) inGainRControl->setVisible(miscExpanded);
    if (wideControl != nullptr) wideControl->setVisible(miscExpanded);
    if (outGainControl != nullptr) outGainControl->setVisible(miscExpanded);
