...
void EqlModuleProcessor::processBlock(juce::AudioBuffer<float>& buffer)
{
                                                          ...
                                                          const int filterIndex)
    {
        ...
        const auto slopeDbPerOct = filterSlopeChoiceParams[bandArrayIndex] != nullptr
            ? static_cast<double>(EqlModuleProcessor::getBellSlopeValueForChoiceIndex(filterSlopeChoiceParams[bandArrayIndex]->getIndex()))
...
EqlModuleProcessor::FilterType EqlModuleProcessor::getFilterTypeForBand(const size_t filterIndex) const noexcept
{
