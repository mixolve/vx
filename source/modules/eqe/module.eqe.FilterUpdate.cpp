...
void EqeModuleProcessor::resetFilters() noexcept
{
...
void EqeModuleProcessor::updateFilters()
{
    const juce::ScopedLock lock(filterProcessLock);
    ...
    for (int filterIndex = 0; filterIndex < maxFilterCount; ++filterIndex)
    {
        ...
        const auto slope = filterSlopeChoiceParams[bandArrayIndex] != nullptr
            ? static_cast<float>(EqeModuleProcessor::getBellSlopeValueForChoiceIndex(filterSlopeChoiceParams[bandArrayIndex]->getIndex()))
