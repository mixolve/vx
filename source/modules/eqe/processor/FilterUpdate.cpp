...
void EqeAudioProcessor::resetBellFilters() noexcept
{
...
void EqeAudioProcessor::updateBellFilters()
{
    ...
    for (int bellIndex = 0; bellIndex < maxBellFilterCount; ++bellIndex)
    {
        ...
        const auto slope = bellSlopeChoiceParams[bandArrayIndex] != nullptr
            ? static_cast<float>(EqeAudioProcessor::getBellSlopeValueForChoiceIndex(bellSlopeChoiceParams[bandArrayIndex]->getIndex()))
