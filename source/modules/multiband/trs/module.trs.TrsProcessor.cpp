#include "module.trs.TrsProcessor.h"
...
inline constexpr auto trsBandOrder = std::to_array<ParameterOrderEntry>({
    { TrsModuleProcessor::paramTransOnId, "TRANS.ON/OFF" },
    { TrsModuleProcessor::paramSusOnId, "SUS.ON/OFF" },
    { TrsModuleProcessor::paramTransGainId, "TRANS.GAIN" },
    { TrsModuleProcessor::paramSusGainId, "SUS.GAIN" },
...
constexpr std::array<const char*, TrsModuleProcessor::numCrossoverSlots> crossoverSuffixes {
...
TrsModuleProcessor::TrsModuleProcessor(juce::AudioProcessor& owner)
    : ownerProcessor(owner),
      parameters(internalParameterHost, nullptr, "trs_state", createParameterLayout())
{
...
juce::String TrsModuleProcessor::makeBandParameterId(const size_t bandIndex, const char* suffix)
{
    return "band" + juce::String(static_cast<int>(bandIndex + 1)) + "_" + suffix;
}

juce::String TrsModuleProcessor::makeFullbandParameterId(const char* suffix)
{
    return "fullband_" + juce::String(suffix);
}

juce::String TrsModuleProcessor::makeSoloParameterId(const size_t bandIndex)
{
    return "soloBand" + juce::String(static_cast<int>(bandIndex + 1));
}

juce::String TrsModuleProcessor::makeActiveSplitCountParameterId()
{
...
juce::AudioProcessorValueTreeState::ParameterLayout TrsModuleProcessor::createParameterLayout()
{
    ...
    for (const auto& entry : trsCrossoverOrder)
    {
        ...
        if (key == "active_split_count")
        {
            crossoverGroup->addChild(floatParam(makeActiveSplitCountParameterId(),
                                                "TRS / CROSSOVER / " + juce::String(entry.label),
    ...
    for (size_t bandIndex = 0; bandIndex < numBands; ++bandIndex)
    {
        ...
        for (const auto& entry : trsBandOrder)
        {
            const auto key = juce::String(entry.key);
...
trs::dsp::DspCore::Parameters TrsModuleProcessor::readBandParameters(const size_t bandIndex) const noexcept
{
...
TrsModuleProcessor::MultibandProcessor::CrossoverFrequencies TrsModuleProcessor::readCrossoverFrequencies() const noexcept
{
...
size_t TrsModuleProcessor::readActiveSplitCount() const noexcept
{
...
TrsModuleProcessor::MultibandProcessor::SoloMask TrsModuleProcessor::readSoloMask() const noexcept
{
